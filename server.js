...
import { onRequest as handleGradeRequest } from "./functions/api/grade.js";
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === "/api/grade") {
      const body = request.method === "POST" ? await readJsonRequest(request).catch(() => ({})) : undefined;
      const apiResponse = await handleGradeRequest({
        request: new Request(`http://localhost:${port}/api/grade`, {
          method: request.method,
          headers: { "Content-Type": "application/json" },
          body: body === undefined ? undefined : JSON.stringify(body)
        }),
